...
bottleneck_dim           = int(sys.argv[1]) #2
n_epoch1                 = 50000
bat_size                 = 956
dropoutRate1             = 0.8
...
history         = caeDual.fit({'x' : logOnePlusGeneExpression}, {'xd1' : logOnePlusGeneExpression, 'z1' : np.zeros((ff, bb))}, batch_size=bat_size, epochs=n_epoch1, validation_data=(heldOut, {'xd1':heldOut, 'z1':np.zeros((heldOut.shape[0], bb))}))
...
fileName                               = os.path.join(root_folder, 'singleAE_6_5byExpression_dim' + sys.argv[1] + '_run' + sys.argv[2] + '_iter50K_0.8Dropout_intermediate100_BN_bat956.mat')
sio.savemat(fileName, {'e1':e1, 'd1':d1, 'et1':et1, 'dt1':dt1, 'sample_id':sample_id, 'val_xd1_loss':val_xd1_loss, 'xd1_loss':xd1_loss})
#saved model weights
modelweights_filename = os.path.join(root_folder, 'singleAE_model0004_weights.h5')
caeDual.save_weights(modelweights_filename)
fileName                               = os.path.join(root_folder, 'kerasModel_singleAE_6_5byExpression_dim' + sys.argv[1] + '_run' + sys.argv[2] + '_iter50K_0.8Dropout_intermediate100_BN_bat956.h5')
